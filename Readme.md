...
## How to run codeon
```shell
codeon create -s 'json_string' [ --hot ] [ -b ]
codeon update -s work_file_name.py [ --hot ] [ -b ]
codeon prompt_info -s work_file_name -i package -v 2
codeon code -s work_file_name.py -p '__CR Prompt__ text or file name' [--hot] [ -b ]
```
...
Note: Do NOT add a cr_id or any fields, other than the fields listed above!

Some technical fields will be added by the engine.
1. **cr_id:** automatically generated by the engine (Do not add this field !)
