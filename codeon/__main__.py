"""
    Entry poiont for codeon shell calls 
    ###################################################################################
    
    __main__.py imports the api module from codeon.apis >> apiModule.py
                and runs it
    ...
    for user info runs: 
        python -m codeon info
    above cmd is identical to
        python -m codeon.apis.info
...
def runable(*args, api, **kwargs):
    ...
    """
    return importlib.import_module(f"codeon.apis.{api}")
